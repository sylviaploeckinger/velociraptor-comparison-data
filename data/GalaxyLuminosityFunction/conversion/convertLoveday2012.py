...
for i in range(5):
    ...
    arg = 0.4 * (M_star[i] - M)
    Phi = unyt.Mpc ** (-3) * 0.4 * np.log(10) * phi_star[i] * ((10.0**arg)**(1.0 + alpha[i])) * np.exp(-10.0**arg)
    M = unyt.unyt_array(M, "dimensionless")

    # Write everything
    processed = ObservationalData()
    processed.associate_x(M, scatter=None, comoving=True, description="Magnitudes %s-band"%bands[i])
