...
sigma_SFR, sigma_SFR_err, sigma_H2, sigma_H2_err = np.genfromtxt(input_filename, unpack=True, usecols=(3,4,7,8))
...
SigmaSFR_err = unyt.unyt_array([10**(binned_data[1]) - 10**(binned_data[1]-binned_data[2]), 10**(binned_data[1]+binned_data[3]) -10**(binned_data[1])], units="Msun/yr/kpc**2")

array_x_bin_std_up = array_of_interest[1:] - binned_data[0]
array_x_bin_std_down = binned_data[0] - array_of_interest[:-1]

SigmaH2_err = unyt.unyt_array([10**(binned_data[0])-10**(binned_data[0]-array_x_bin_std_down), 10**(binned_data[0]+array_x_bin_std_up)-10**(binned_data[0])], units="Msun/pc**2")

processed.associate_x(SigmaH2, scatter=SigmaH2_err, comoving=False, description="$\\Sigma_{\\rm H_2}$")
