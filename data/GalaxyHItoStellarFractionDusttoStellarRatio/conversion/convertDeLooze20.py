...
input_filename = "../raw/JINGLE_MHIMstar_vs_MdustMstar.dat"
delimiter = " "
...
for i in range(len(labels)):
    ...
    logD2S_med = pow(10, raw[:, 1]) * unyt.dimensionless
    logD2S_lo = pow(10, raw[:, 3]) * unyt.dimensionless
    logD2S_hi = pow(10, raw[:, 5]) * unyt.dimensionless

    # Define the scatter as offset from the mean value
    y_scatter = unyt.unyt_array(
        (logMHIMstar_med - logMHIMstar_lo, logMHIMstar_hi - logMHIMstar_med)
    )
    x_scatter = unyt.unyt_array((logD2S_med - logD2S_lo, logD2S_hi - logD2S_med))
    ...
    name = "MHI/Mstar as a function of dust-to-stellar mass ratio"
    plot_as = "points"
    redshift = 0.0
    redshift_lower = 0.0
    redshift_upper = 3.0
