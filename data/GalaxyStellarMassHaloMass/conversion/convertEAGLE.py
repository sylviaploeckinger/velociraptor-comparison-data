...
for halo, latex_name in zip(haloes, latex_names):
    ...
    for z in redshifts:
        ...
        data = np.loadtxt(
            f"../raw/EAGLE_SMHM_{f'{halo:s}'}_L100N1504_{f'z{z:03.1f}'.replace('.', 'p')}.txt"
        )
        M_halo = unyt.unyt_array(data[:, 0], units=unyt.Solar_Mass)
        M_star_16 = unyt.unyt_array(data[:, 1], units=unyt.Solar_Mass)
        M_star_50 = unyt.unyt_array(data[:, 2], units=unyt.Solar_Mass)
        M_star_84 = unyt.unyt_array(data[:, 3], units=unyt.Solar_Mass)
