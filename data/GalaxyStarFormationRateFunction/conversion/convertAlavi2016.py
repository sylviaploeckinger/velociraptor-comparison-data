...
def load_file_and_split_by_z(raw_file_name):
    ...
    for isl, lines in enumerate(split_lines):
        uv_lf_arr.append(np.genfromtxt(lines, usecols=(0, 1, 3, 4)))
...
def process_for_redshift(z, sfrf_at_z):
    ...
    citation = "Alavi et al. (2016)"
    bibcode = "2016ApJ...832...56A "
    name = "SFRF from HST Frontier Fields UV luminosity function"
    plot_as = "points"
    redshift = z
    h = cosmology.h
    ...
    # the median point 'y'
    Phi_err = 1e-2 * sfrf_at_z[:, [3, 2]].T * unyt.Mpc ** (-3)
...
input_filename = "../raw/Alavi2016.txt"
delimiter = "\t"

output_filename = "Alavi2016_{}.hdf5"
output_directory = "../"
